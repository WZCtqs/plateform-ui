...
<script>
import { listClasses, getClasses, delClasses, addClasses, updateClasses, exportClasses, statesModif, lineTypeid, squadNumber } from "@/api/trains/classes";
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(row) {
      ...
      this.getNum();
      this.form.classEtime = timestampToTime(new Date(this.form.classStime).getTime() + row.classTTransporttime * 24 * 60 * 60 * 1000) + "00:00:00";
      this.agreeChange();
    ...
    timeChange() {
      if (this.form.classStime && this.form.classTransporttime) {
        this.form.classEtime = timestampToTime(new Date(this.form.classStime).getTime() + this.form.classTransporttime * 24 * 60 * 60 * 1000) + "00:00:00";
      }
    },
    //计划开车时间为空弹出提示
    linefire() {
    ...
    handleCheckedCitiesChangeFull(value) {
      ...
      this.isIndeterminateFull = checkedCount > 0 && checkedCount < this.siCheckbox.length;
    },
    //散货站点的选择事件
    handleCheckedCitiesChangeLcl(value) {
      let checkedCount = value.length;
      this.checkAllLcl = checkedCount === this.siCheckbox.length;
      this.isIndeterminateLcl = checkedCount > 0 && checkedCount < this.siCheckbox.length;
    },
    //获取站点信息
    agreeChange() {
      const id = this.form.lineId;
      lineTypeid(id).then(response => {
