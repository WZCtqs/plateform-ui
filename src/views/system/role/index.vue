    ...
    <el-dialog :title="title" :visible.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
         <el-form-item label="节点位置"
                          prop="parentRoleId">
              <treeselect v-model.trim="form.parentRoleId"
                          :options="roleOptions"
                          placeholder="选择节点位置" />
            </el-form-item>
        <el-form-item label="角色名称" prop="roleName">
...
<script>
import { listRole, getRole, delRole, addRole, updateRole, exportRole, dataScope, changeRoleStatus, treeselect as roleTreeselect } from "@/api/system/role";
  ...
  methods: {
    ...
    getList() {
      ...
      listRole(this.addDateRange(this.queryParams, this.dateRange)).then(
        response => {
          this.roleList = response.data; // 大列表数组
          this.total = response.data.length; // 大列表数组长度
          this.loading = false;
        }
      ).catch(()=>{
          this.loading = false;
    ...
    getRoleMenuTreeselect(roleId) {
      ...
      menuTreeselect().then(response => {
        this.menuOptions = response.data;
        this.$refs.menu.setCheckedKeys(data1);
      });
    ...
    handleUpdate(row) {
      ...
      getRole(roleId).then(response => {
        ...
        if(this.form.roleKey === 'admin'){
            this.msgError("不允许操作超级管理员角色")
        }else {
            if (this.form.parentRoleId === 0 && this.form.ancestors === '0') {
                this.form['parentRoleId0'] = this.form.parentRoleId
                this.form.parentRoleId = this.form.roleId
            }
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if(this.form['parentRoleId0'] === 0 && this.form.ancestors === '0'){
            this.form.parentRoleId = 0
            delete this.form.parentRoleId0
          }
