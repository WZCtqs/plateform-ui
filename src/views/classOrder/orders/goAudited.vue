<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="70px">
      ...
      <el-form-item label="订舱方" prop="clientUnit">
       ...
       <el-form-item label="往返" prop="classEastandwest">
        <el-select v-model.trim="queryParams.classEastandwest" placeholder="请选择" style="width:100px;" clearable size="mini">
     ...
     <el-form-item label="路线" prop="lineTypeid">
        <el-select v-model.trim="queryParams.lineTypeid" placeholder="请选择" style="width:100px;" clearable size="mini">
          <el-option label="-请选择-" value />
    ...
    <el-table
      :class="this.heightScreen===600?'y1 orderTable':'y2 orderTable'"
      v-loading="loading"
      :data="list"
      border
      ref="multipleTable"
    ...
    >
      ...
      <el-table-column label="站点" align="center" prop="lineName" />
     <el-table-column label="班列日期" align="center" width="100">
      ...
      <!-- <el-table-column label="状态" align="center" class-name="small-padding fixed-width">
       ...
       <el-table-column label="驳回原因" align="center" class-name="small-padding fixed-width" width="120">
        <template slot-scope="scope">
          <span style="color:#f00;cursor:pointer;" v-if="!!scope.row.refuseInfo && scope.row.isread === '0'" @click="bohuiReason(scope.row)">未读</span>
          <span style="color:green;cursor:pointer;" v-if="!!scope.row.refuseInfo && scope.row.isread === '1'" @click="bohuiReason(scope.row)">已读</span>
          <span v-if="!scope.row.refuseInfo">暂无</span>
        </template>
      </el-table-column>
      <el-table-column label="转待修改记录" align="center" class-name="small-padding fixed-width" width="120">
        <template slot-scope="scope">
           <el-button
            size="mini"
            type="text"
         ...
         <li v-for="(item,index) in isupdateList.list" :key="index">{{item}}</li>
          ...
          <h5 class="h5Css0">转待审核</h5>
              <el-row>
                 <el-col :span="20">
                   <!-- 整箱显示 -->
                  <el-form-item label="转待审核" prop="isexamline">
                    <el-radio v-model.trim="form1.isexamline" label="1">审核通过</el-radio>
                    <el-radio v-model.trim="form1.isexamline" label="3">审核失败</el-radio>
                    <!-- <span class="msgSpancss0 redTxt">提示：默认起运港收费，目的港收费(请特别备注)，发车后，不再更改。</span> -->
                  </el-form-item>
                </el-col>
                <el-col :span="20" v-if="form1.isexamline === '3'">
                ...
                <el-col :span="20" v-if="form1.isexamline === '1'">
                   <!-- 整箱显示 -->
                  <el-form-item label="是否有改单费" prop="dcGaidanState">
                    <el-radio v-model.trim="form1.dcGaidanState" label="是">是</el-radio>
                    <el-radio v-model.trim="form1.dcGaidanState" label="否">否</el-radio>
                    <!-- <span class="msgSpancss0 redTxt">提示：默认起运港收费，目的港收费(请特别备注)，发车后，不再更改。</span> -->
                  </el-form-item>
                </el-col>
                <el-col :span="20" v-if="form1.isexamline === '1'">
                  ...
                  <el-form-item label="舱位号" prop="orderNumber">
                    <el-input v-model.trim="form1.orderNumber" />
                  </el-form-item>
                </el-col>
                <el-col :span="20" v-if="form1.isexamline === '1'">
...
<script>
import { fetchList1,detailapi,getSiteByOrderId, orderCheckReplyapi, checkEditRecord,orderCancelapi,stationList,stationNewNum,goodsInfo,addIsRead } from "@/api/classOrder/orders";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchList1(this.queryParams).then(response => {
    ...
    /** 审核弹框打开 */
     handleShenhe() {
       ...
       this.form1 = {
        ...
        orderNumber:this.arrs[0].orderNumber,
        classEastandwest:this.arrs[0].classEastandwest
      }
      //获取最新的场站编号
      stationNewNum(this.arrs[0].orderId).then(res=>{
        this.orderUploadcode=res
        //场站地址
      let dccs={
        isconsolidation:this.arrs[0].isconsolidation,
        orderUploadcode:this.orderUploadcode,
      }
      ...
      //修改记录
      let params={orderId:this.arrs[0].orderId}
    ...
    shenheFn() {
      ...
      if(this.form1.isexamline === '1'){
          ...
          params.examInfo=null  // 申请转待原因
          params.isexamline='1'  // 操作状态（传定值1） 
          params.orderId=this.ids[0] // 订单id
          params['orderNumberNew']= this.form1.orderNumber
          params['stationidNew']= this.form1.stationid
          // params={
          //   createuserid: this.$store.getters.id,
          //   createusername: this.$store.getters.name,
          //   dcGaidanState:this.form1.dcGaidanState ,
          //   examInfo:null,
          //   isexamline:this.form1.isexamline ,
          //   orderId:this.form1.orderId
          // }
      }else if(this.form1.isexamline === '3'){
          params['createuserid']= this.$store.getters.id
          params['createusername']= this.$store.getters.name
          params.dcGaidanState= null, 
          params.examInfo=this.form1.examInfo  // 申请转待原因
          params.isexamline='3'  // 操作状态（传定值3） 
          params.orderId=this.ids[0] // 订单id
          params['orderNumberNew']= null
      }
       this.$confirm(
        '是否确认要执行此操作吗?',
        "警告",
        ...
        .then(function() {
          return orderCheckReplyapi(params);
        })
        .then((res) => {
          if(res.code === 200){
            this.getList();
